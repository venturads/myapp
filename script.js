...
function getdata(data){
  ...
  for(let i=0;i<data.posts.length;i++){
    list += "<div class='btn-group-justified content'><button href='#' type='button' class='btn btn-info' id='settings' data-toggle='collapse' data-target='#demo" + data.posts[i].id + "'>" +  data.posts[i].title + "</button><div id='demo" +  data.posts[i].id + "' class='collapse'>" + data.posts[i].title + data.posts[i].content + "</div></div>";
    }
  document.getElementById("root").innerHTML = list;
}
